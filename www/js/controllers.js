...
.controller('AppCtrl', function($scope, $ionicModal, $ionicPopover, $ionicPlatform, $localStorage) {
  ...
  // $scope.$storage.url = 'http://mlollo.rmorpheus.enseirb.fr:8080'
...
.controller('MenuCtrl', function($scope, $ionicPopup, $ionicHistory, 
  ...
  $scope.toggleInfo = function(info) {
    ...
    if($scope.isInfoShown(info)) {
      ...
      $scope.profillist._info.forEach(function(value,key){
        $scope.showLocation(value);
      });
  ...
  // switch menu view to see your position history and your informations -----------------
  ...
  $scope.onSearchCancel = function(data){
    data.search = '';
    $scope.reloadFriendsList();
  };

  $scope.onSearchCancelProfil = function(data){
    data.search = '';
    $scope.onProfil({'search' : $scope.$storage.pseudo});
  };
  ...
  // method that can refresh every 100 secondes the data on the side menu
  ...
  $scope.showStorage = function($event){
